...
let altNavBar = document.getElementById('altNavBar');

window.onload = function () {
    if(window.location == 'http://localhost/html/LaTriade/menu.php') {
    ...
    for(let child of altNavBar.children) {
...
function animateMenuBtn(container) {
    container.classList.toggle('change'); //toast anim

    if(altNavBar.style.width == '75%') {
        for(let child of altNavBar.children) {
            child.style.opacity = '0%';        
            child.style.display = 'none';    
        }
        setTimeout(function() {
            altNavBar.style.width = '0%';
        }, 750);
    } else {
        altNavBar.style.width = '75%';
        setTimeout(function() {
            for(let child of altNavBar.children) {
...
function showPrenotazioni(parent) {
    for(let i of parent.children) {
        i.classList.toggle('hide');
        i.classList.toggle('scrittura');
    }
}